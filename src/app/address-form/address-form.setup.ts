import { apply, FormValueControl, pattern, required, schema, SchemaPath } from "@angular/forms/signals";
...
export const addressSchema = schema<AddressData>((path) => {
    ...
    pattern(path.zipCode, /^\d{5}(-\d{4})?$/ ,{ message: 'Enter a valid zip code.'});
