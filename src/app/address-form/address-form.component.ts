...
@Component({
  ...
  imports: [FormField, FieldErrorComponent, FormRoot],
})
export class AddressFormComponent implements FormValueControl<AddressData>{
  ...
  readonly form = form(this.value, (path) => {
    ...
    pattern(path.zipCode, /^\d{5}(-\d{4})?$/ ,{ message: 'Zip code must be 5 digits or 9 digits with a hyphen.'});
    required(path.city, { message: 'City is required.' });
  });

  constructor(){
    effect(() => {
      if (this.form().touched()) this.touch.emit();
    })
  }
}
